...
3. **F9 en el probe** — dump **manual** al `UE4SS.log` si el perfil no existe o no cuadra. No en el

   bucle ~20 Hz.

`main.lua` en producción: `GetDrivableActor().PowerBrakeHandle` (caché). Python 323:
`CLASS323_PBH_INPUT_BY_NOTCH` en `tsw_command_bus.py`.
...
- `direct_control` / `api_control` → **`controls`**: nombre UObject (`PowerBrakeHandle`, `Throttle`,
...
IPC sigue siendo **muesca/8** (0…1). Escritura nativa de producción:
`SetCurrentOutputValue(muesca−4)`. Si el HUD no se mueve, el probe (`20260828o`) prueba el handshake
Liah: `BeginChangingVHIDComponent` + `SetCurrentInputValue` con los peldaños de abajo (no
