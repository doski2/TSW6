...
sale un solo `BrakeCommand` (`command_from_target`). No hay P2.
...
Cómo **añadir** reglas: [BRAKE_V2 — Cómo tocar reglas](BRAKE_V2.md#cómo-tocar-reglas-sin-borrar-v2)
(fases A límites → B estación → C cluster; no borrar v2).
Paridad de corte Dastsc:
[DASTSC_PARITY.md](DASTSC_PARITY.md#reglas-de-conducción-revisión-2026-08-29).
...
- `HORIZON_SLACK_M = 25` — no B1 de andén hasta horizonte de **servicio** (decel 100 %, no B1 a 1,1

  km)

- `TARGET_CLUSTER_GAP_M = 350` — cartel y andén “juntos”
- `uni=Y` — gap corto: no cabe 55 → soltar → parar en el hueco; el cartel marca el toque, el andén

  el v→0 **cuando entra horizonte**
