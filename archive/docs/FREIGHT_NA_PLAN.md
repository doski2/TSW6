...
**Archivos:** `freight_learner.py`, `online_learner.py`, `governor_physics.py`, `learn_monitor.py`,
`test_freight_learner.py`
...
- [ ] `handle_controller.py` — rama `freight_na` con RPC `power`, `train_brake`, `ind_brake`,
...
**Criterio de aceptación:** a 15 mph con a_target=0.15, elige muesca mínima según perfil (no siempre
máx).
...
| `diag_controles.bat`                                    | Lanzador Fase 0 (monitor API)                        |
| `archive/railbridge/control_diag.py`                    | **Archivado** — diagnóstico companion RailBridge     |
| `train_labels.py`                                       | Etiquetas de mandos UK/freight                       |
...
| `tsw_telemetry_source.py`                               | **Activo** — telemetría HTTPAPI                      |
| `archive/railbridge/tsw_connection.py`                  | **Archivado** — companion SSE                        |
| `TSW_API_V2.md`                                         | **V2** — frenos vía API TSW directa                  |
| `INVESTIGACION.md`                                      | **Dirección actual** — sin RailBridge; UE4SS + Dastsc|
...
**En paralelo:** ver `INVESTIGACION.md` (arquitectura sin RailBridge) y `TSW_API_V2.md` (escritura HTTP frenos).
