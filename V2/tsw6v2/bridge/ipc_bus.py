"""Bus IPC SendCommand — contrato D2 (CANAL_CONTROL). Sin dependencias de tsw6."""
...
def parse_send_ack_line(line: str) -> Optional[dict[str, Any]]:
    """``path:value:ok`` o ``path:value:ok:cmd_id``."""
...
def format_send_command_line(
    ...
    *,
    ...
    path = resolve_control_path(control, schema)
    if not path:
...
def write_send_command(
    ...
    *,
    ...
    path_name = resolve_control_path(control, schema)
    if not path_name or not is_allowed_path(path_name, schema):
        return False
