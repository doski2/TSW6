...
:: ── 7. Ya hay BD? ────────────────────────────────────────────────────────
if exist "%~dp0tsw_hud.db" (
    echo.
    echo  [OK] Ya existe %~dp0tsw_hud.db
    %PY% verificar_hud_db.py
    echo.
...
echo        tsw_hud.db  --^>  %~dp0tsw_hud.db
echo      ^(o usa el boton de sincronizar BD si aparece^)
echo   5. Cierra hud.exe y ejecuta:  python verificar_hud_db.py
echo.
